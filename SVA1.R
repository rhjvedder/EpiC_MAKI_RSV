...
setwd(loc.out)
load(file="MAKI_trimmed_M.Rdata")
M_matrix<- M.val
...
PHENO <- phenotype[,c("Prevention", "Batch", "Age", "Gender")]
...
M_matrix <- t(M_matrix)
system.time(ind.res <- mclapply(setNames(seq_len(ncol(M_matrix)), dimnames(M_matrix)[[2]]), GLMtest, meth_matrix=M_matrix, Y=PHENO$Prevention, X1=PHENO$Batch, X2=PHENO$Age, X3=PHENO$Gender, SV<- SVs, mc.cores=12))
...
save(p.all,file=paste0(filename,".Rdata"))
...
qqPlot(pvalue, main="QQ of methylation model Prevention = Meth + Batch")
...
write.csv(cpg, file=paste0("cpg_", filename, ".csv"))
save(cpg, file=paste0("cpg_", filename, ".Rdata"))
#qqman
Ci<- apply(all.results, 1, function(x)  which(annot$Name==x[1]))
bp <- as.numeric(annot$pos[Ci])
p <- as.numeric(all.results[,4])
chr <- as.character(annot$chr[Ci])
...
manhattan(cpg.all, main="manhattan of methylation model Prevention = Meth + Batch")
dev.off()
