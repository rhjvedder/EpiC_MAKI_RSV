...
sink("/data/f114798/Logs/log_norm_plots.log")
start.time <- Sys.time()
...
norms.sets <- sapply(norms, function(norma) {
  load(paste(loc.out, paste("epic_maki_", norma, "_meth_set.Rdata"), sep = "/"))
  return(m.set.sq)
})
...
# plots showing normalizing before and after, consider making seperate plots for the sample plates
  for (plate in levels(factor(targets$Sample_Plate))) {
    ...
    png(paste(loc.out.norm, paste(norma, "_probeType_", plate, ".png"), sep = "/"))
    par(mfrow=c(2,3))
    for (n in 1:6) {
...
for (norma in norms) {
  ...
  save(cormat, file=paste(loc.out.norm, paste(norma, "_heatmap_cor.png"), sep="/"))
...
png(file=paste(loc.out.norm, "boxplot_controls.png", sep="/"))
ggplot(dat) + 
