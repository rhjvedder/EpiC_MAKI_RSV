...
loc.model <- "Data/Models/model_6_Prevention"
setwd(loc)
...
M_matrix <- t(M_matrix)
system.time(ind.res <- mclapply(setNames(seq_len(ncol(M_matrix)), dimnames(M_matrix)[[2]]), GLMtest, meth_matrix=M_matrix, Y=PHENO$Prevention, X1=PHENO$Batch, X2=PHENO$Age, X3=PHENO$Gender, X4=PHENO$SmokingPregnancy, SV<- SVs, mc.cores=12))
...
# chr
filename <- "MAKI_trimmed_M_glm_model_6"
