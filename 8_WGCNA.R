...
phenotype <- data.frame(Sample=phenotype[,2], Wheeze=phenotype[,20], Asthma=phenotype[,21], Prevention=phenotype[,27], FEV05=phenotype[,26], stringsAsFactors=FALSE)
n <- length(targets$Sample_Name)
phenotype1 <- data.frame(Basename=rep(NA, n), Sample=rep(NA, n), Wheeze=rep(NA, n), Asthma=rep(NA, n), Prevention=rep(NA, n), FEV05=rep(NA, n), stringsAsFactors=FALSE)
...
for (i in 1:n) {
  ...
  phenotype1[i,] <- c(x[8], x[1], y[2], y[3], y[4], y[5])
}
...
ADJ1=abs(cor(M_matrix,use="p"))^6
# When you have a lot of genes use the following code
k=softConnectivity(datE=M_matrix,power=6)
# Plot a histogram of k and a scale free topology plot
png(paste(loc.wcna, "SFT_hist.png", sep="/"), width=800, height=800)
par(mfrow=c(1,2))
hist(k)
...
sft = pickSoftThreshold(M_matrix, dataIsExpr=TRUE, powerVector=powers, verbose=5, networkType="unsigned")
...
beta <- 16
net <- blockwiseModules(M_matrix, power=beta, TOMType="unsigned", minModuleSize=30, reassignThreshold=0, mergeCutHeight=0.25, numericLabels=TRUE, pamRespectsDendro=FALSE, saveTOMs=TRUE, saveTOMFileBase="maki_tom", verbose=3)
