...
loc.meth <- "Data/meth_set"
setwd(loc)

# loading data
load(file=paste(loc.rgdata, "Detection_Pvalues.Rdata", sep="/"))
...
bad.probes <- rowMeans(all.det.p > 0.01) > 0.1
table(bad.probes)
bad.probe.names.detP <- rownames(all.det.p[bad.probes,])

save(det.p, file=paste(loc.rgdata, "epic_maki_detP.Rdata", sep="/"))
save(rg.set, targets, file=paste(loc.rgdata, "epic_maki_rgSet_postDetP.Rdata", sep="/"))
#save 
...
save(m.set, targets, file=paste(loc.meth, "epic_maki_mSet.Rdata", sep="/"))
save(gm.set, targets, file=paste(loc.meth, "epic_maki_gmSet.Rdata", sep="/"))
...
png(paste(loc.qc, "pca_x_gender_prcomp.png", sep = "/"))
ggplot(pca.out$x,aes(x=PC1,y=PC2,col=x.pheno)) + geom_point(size=3,alpha=0.5) + labs(title="PCA plot of Gender, no controls, X chromosomes", x=paste("PC1 (", round(prop[1]*100, 2), "%)"), y=paste("PC2 (", round(prop[2]*100, 2), "%)"), colour="Gender")
dev.off()

# select out any samples which aren't grouped in with male or female
bad.quality.selection <- rownames(x.beta)[(pca.out$x[,1] > -4) & (pca.out$x[,1] < 10)]
...
bad.samples.trail1 <- gm.set$Basename[!samples.good]
...
for (row in 1:nrow(bad.samples.data)) {
  png(paste(loc.qc, "Gender_histograms", "bad_samples", paste(bad.samples.trail[row], "_", bad.sample.genders[row], "_histogram_x.png"), sep = "/"))
...
for (row in 1:nrow(y.beta)) {
  png(paste(loc.qc, "Gender_histograms", "misgendered", paste(bad.samples.trail[row], "_", bad.sample.genders[row], "_histogram_y.png"), sep = "/"))
...
for (row in 1:nrow(good.samples.data)) {
  png(paste(loc.qc, "Gender_histograms", "good_samples", paste("G_", good.samples.trail[row], "_", good.samples.genders[row], "_histogram_x.png"), sep = "/"))
...
for (row in 1:nrow(y.beta)) {
  png(paste(loc.qc, "Gender_histograms", "good_samples", paste("G_", good.samples.trail[row], "_", good.samples.genders[row], "_histogram_y.png"), sep = "/"))
...
for (row in 1:nrow(bad.quality.data)) {
  png(paste(loc.qc, "Gender_histograms", paste(bad.quality.trail[row], "_", bad.sample.genders[row], "_histogram_x.png"), sep = "/"))
...
# print y chromosome
for (row in 1:nrow(y.beta)) {
  png(paste(loc.qc, "Gender_histograms", "misgendered", paste(bad.quality.trail[row], "_", bad.sample.genders[row], "_histogram_y.png"), sep = "/"))
...
write.csv(bad.quality.rs, file=paste(loc.qc, "pca_center_RS.csv", sep="/"))
...
write.csv(bad.samples.asthma, file=paste(loc.qc, "bad_samples_asthma.csv", sep="/"))

png(paste(loc.qc, "bad_Samples_asthma_Table.png", sep = "/"), width = 800, height = 400, units = "px")
...
qc.probes.colnames <- c("Detect p-value", "Probes X/Y", "Cross Reactive", "Polymorphic Pr")
...
pdf(paste(loc.qc, "qc_Sample_Table.pdf", sep = "/"), height=11, width=8.5)
grid.table(qc.s.df)
dev.off()
print("---------------------------------------------------------------------")
...
write.csv(data.frame(Sample=c(nam_sampl(bad.sample.names.detP), nam_sampl(bad.samples.trail1)), Type=c(rep("det p", length(bad.sample.names.detP)), rep("misgendered", length(bad.samples.trail1)))), file=paste(loc.qc, "bad_samples.csv", sep="/"))
print("---------------------------------------------------------------------")
qc.p.df <- data.frame(nbr_sampl(bad.probe.names.detP), nbr_sampl(bad.probe.names.xy), nbr_sampl(bad.probe.names.cr), nbr_sampl(bad.probe.names.pm))
...
for (d in names(qc.n.df)) {
  ...
  write.csv(n, file=paste(loc.qc, paste(d, "filtered_probes.csv", sep="_"), sep="/"))
}
colnames(qc.p.df) <- qc.probes.colnames
pdf(paste(loc.qc, "qc_Probe_Table.pdf", sep = "/"), height=11, width=8.5)
grid.table(qc.p.df)
dev.off()
