...
RLMtest= function(methcol, meth_matrix, Y, X1) {
  
  mod= rlm(as.numeric(Y)~meth_matrix[, methcol]+X1,maxit=200)
...
system.time(ind.res <- mclapply(setNames(seq_len(ncol(M_matrix)), dimnames(M_matrix)[[2]]), RLMtest, meth_matrix=M_matrix, Y=PHENO$FEV05, X1=PHENO$Batch, mc.cores=12))
...
# chr
filename <- "MAKI_trimmed_M_glm_model_7"
...
qqPlot(pvalue, main="QQ of methylation model FEV05 = Meth + Batch")
